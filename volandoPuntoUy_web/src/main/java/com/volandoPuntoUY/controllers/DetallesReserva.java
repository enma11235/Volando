...
public class DetallesReserva extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse resp)
		...
		if (configFile.exists()) {
		    try (FileInputStream input = new FileInputStream(configFile)) {
		        ...
		        System.err.println("detallesReserva: Error al cargar el archivo de configuración. Usando valores predeterminados.");
		    }
		} else {
		    System.out.println("detallesReserva: Archivo de configuración no encontrado. Usando valores predeterminados.");
		}

		// Construir la URL del servicio
		String serviceUrl = "http://" + ipStr + ":" + portStr + "/publicador?wsdl";
		//System.out.println("detallesReserva: Servicio buscado en " + serviceUrl);
	...
	protected void doDownloadTarjetaEmbarque(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            // Obtener la fecha y hora del embarque
            String fechaHoraFormateada = Integer.toString(LocalDate.parse(reserva.getEmbarque().getFechaEmbarque()).getDayOfMonth()) + "/" + Integer.toString(LocalDate.parse(reserva.getEmbarque().getFechaEmbarque()).getMonthValue()) + "/" + Integer.toString(LocalDate.parse(reserva.getEmbarque().getFechaEmbarque()).getYear()) + "   " + Integer.toString(LocalTime.parse(reserva.getEmbarque().getHoraEmbarque()).getHour()) + ":" + Integer.toString(LocalTime.parse(reserva.getEmbarque().getHoraEmbarque()).getMinute());
            ...
            if (vuelo != null) {
                ...
                tablaInfo.addCell(Integer.toString(Duration.parse(vuelo.getDuracion()).toHoursPart()) + " horas y " + Integer.toString(Duration.parse(vuelo.getDuracion()).toMinutesPart())+" minutos");
            } else {
            ...
            parrafoReserva.setSpacingAfter(10); // Espacio después del título
            ...
            if (reserva != null && reserva.getPasajes() != null) {
                ...
                for (DtPasaje p : reserva.getPasajes()) {
                    ...
                    tablaInfo.addCell(String.valueOf(p.getAsiento()));
                    i++;
                }
                document.add(tablaInfo);
            }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        else if("realizar".equals(action)) {
