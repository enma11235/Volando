...
public class Home extends HttpServlet {
	...
	public static void initSession(HttpServletRequest request) {
        ...
        String aeroParam = request.getParameter("aero");


        if (categoriaParam != null) {
            session.setAttribute("categoria", categoriaParam);
        } else {
            session.setAttribute("categoria", "Internacionales");
        }

        DtRutaDeVueloWebArray rutasAMostrar = null;

        if (aeroParam != null) {
            try {
				rutasAMostrar = port.listarRutasConfirmadasDTWeb(aeroParam);
			} catch (UsuarioNoEsAerolineaExcepcion_Exception e) {
				e.printStackTrace();
			}
        } else {
            rutasAMostrar = new DtRutaDeVueloWebArray();
            ...
            for (DtRutaDeVueloWeb rv : rutasConf.getItem()) {
                ...
                for (DtCategoria c : rv.getCategorias()) {
                    if (c != null && c.getNombre().equals(session.getAttribute("categoria"))) {
                        hasMatchingCategory = true;
                        break;
                    }
                }
                if (hasMatchingCategory) {
                    rutasAMostrar.getItem().add(rv);
                }
            }
        }
        ...
        try {
            session.setAttribute("aerolineasUsuarios", port.listarAerolineasDataWeb());
