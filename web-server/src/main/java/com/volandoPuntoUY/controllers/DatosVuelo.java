    ...
    protected void processRequest(HttpServletRequest req, HttpServletResponse resp)
		...
		if (configFile.exists()) {
		    try (FileInputStream input = new FileInputStream(configFile)) {
		        ...
		        System.err.println("datosVuelo: Error al cargar el archivo de configuración. Usando valores predeterminados.");
		    }
		} else {
		    System.out.println("datosVuelo: Archivo de configuración no encontrado. Usando valores predeterminados.");
		}

		// Construir la URL del servicio
		String serviceUrl = "http://" + ipStr + ":" + portStr + "/publicador?wsdl";
		System.out.println("datosVuelo: Servicio buscado en " + serviceUrl);
        ...
        List<DtReservaWeb> reservasVuelo = new ArrayList<DtReservaWeb>();
        for(String reser : port.listarReservasWeb(nomVuelo).getItem()){
            reservasVuelo.add(port.obtenerInfoReservaWeb(nomVuelo, reser));
        }

        req.getSession().setAttribute("reservasVuelo", reservasVuelo);
